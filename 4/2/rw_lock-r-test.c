...
void init_rwlock(struct rw_lock * rw)
{
   ...
   rw->num_Reader_Threads = 0; //읽기 쓰레드 개수 초기화
   rw->num_Writer_Threads = 0; //쓰기 쓰레드 개수 초기화
   rw->num_Waiting_Writer_Threads = 0; //기다리고 있는 쓰기 쓰레드 개수 초기화
...
void r_lock(struct rw_lock * rw)
{
   pthread_mutex_lock(&(rw->mutex)); //락 걸기
	//쓰기 쓰레드가 하나라도 일하고 있을 경우
   while(rw->num_Writer_Threads>0)

...
void ssufs_readSuperBlock(struct superblock_t *superblock){
	...
	*/
	char tempBuf[BLOCKSIZE]; //superblock담을 버프
	lseek(DISK_FD, 0, SEEK_SET); //맨 첨 위치(superblock 읽기 위해)
	int ret = read(DISK_FD ,tempBuf, BLOCKSIZE); //superblock읽어서 tempBuf에 저장
	assert(ret == BLOCKSIZE); //superblock제대로 읽지 못했다면 중단!
	memcpy(superblock, tempBuf, sizeof(struct superblock_t)); //superblock에 버퍼있던거 넣어주기
}
...
void ssufs_writeSuperBlock(struct superblock_t *superblock){
	...
	*/
	char tempBuf[BLOCKSIZE]; //superblock담을 버프
	memcpy(tempBuf, superblock, sizeof(struct superblock_t)); //인자로 받은거 버퍼에 저장
	lseek(DISK_FD, 0, SEEK_SET); //맨 첨 위치(superblock 쓰기 위해)
	int ret = write(DISK_FD ,tempBuf, BLOCKSIZE); //쓰기!
	assert(ret == BLOCKSIZE); //제대로 쓰지 못했다면 중단!
}
...
void ssufs_formatDisk(){
	...
	*/
	...
	FILE* fp;
	fp = fopen("ssufs", "w+"); //ssufs오픈, 없으면 만들어
	DISK_FD = fileno(fp); //fd 얻어서 DISK_FD에 저장

	// superblock 초기화
	struct superblock_t *superblock = (struct superblock_t *)malloc(sizeof(struct superblock_t));
	memcpy(superblock->name, "ssufs", 8); //superblock 구조체의 name 부분에 ssufs 넣기
	for(int i=0; i<NUM_INODES; i++) //inodeblock 다 FREE로 비워주기
		superblock->inode_freelist[i] = INODE_FREE;
	for(int i=0; i<NUM_DATA_BLOCKS; i++){ //datablock 다 FREE로 비워주기
		superblock->datablock_freelist[i] = DATA_BLOCK_FREE;
	}
	ssufs_writeSuperBlock(superblock); //ssufs에 superblock 써주기
	free(superblock); //ssufs에 superblock 썼으니 free해서 할당해제해주기
	
	// inode 초기화
	struct inode_t *inode = (struct inode_t *)malloc(sizeof(struct inode_t));
	memcpy(inode->name, "", 1); //inode이름 공백으로 비우기
	inode->status = INODE_FREE; //inode 상태 FREE!
	inode->file_size = 0;  //file size 0
...
void ssufs_writeInode(int inodenum, struct inode_t *inodeptr){
	...
	*/
	assert(inodenum < NUM_INODES); //가리키는 inode가 범위 벗어나면 중단!
	char tempBuf[BLOCKSIZE / NUM_INODES_PER_BLOCK]; 
